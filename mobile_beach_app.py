...
def create_mobile_map(df, weather_cache):
    ...
    for _, row in df.iterrows():
        ...
        tooltip_text = f"🏖️ {row['Name_English']}\n📍 {row.get('Municipality_English', '')}"
    ...
    deck = pdk.Deck(
       map_style='mapbox://styles/mapbox/outdoors-v11',
        ...
        layers=[layer],
        tooltip={
            "html": "<b>{name}</b><br/><small>{municipality}</small><br/><pre style='white-space: pre-wrap; font-family: Arial;'>{tooltip}</pre>",
            "style": {
...
def main():
   ...
   # Mobile-optimized header with Blue Flag image
    ...
    st.pydeck_chart(mobile_map, use_container_width=True)
